...
- Zod-validated popup settings (`extension/src/popupSettings.js`). `enabled`, `displayMode`, and `flipOverride` now go through a typed schema with field-by-field repair so a single bad storage key doesn't drop the rest.
- Persisted flip-override (PR #28): the popup's "I'm playing white/black" choice is stored in `chrome.storage.local` and locks `getPlayerColor()` in the content script before any adapter heuristic runs. Survives `findBoard()` resets so the override holds across game switches.
- `@types/express` + `@types/ws` + `@types/chrome` as devDeps (PR #29). `httpRoutes.js` and `pin.js` now annotate Express handlers with the real `Request` / `Response` / `NextFunction` shapes; the `HttpRoutesDeps.express` field is `typeof import('express')` instead of `any`.
